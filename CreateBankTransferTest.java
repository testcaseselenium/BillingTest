...
/**
 *Test ID : 8333 - VWI-17244-CP232-SIT_Payment_200011_TC1_Create a disbursement 
 * 
 * @author CMONTE5
 *
 */
public class CreateBankTransferTest {
	...
	public void accountCharges() throws InterruptedException, IOException {
		...
		String unapliedamountText = readFile.getCellValue(filepath, "Sheet6", 2, 1);
		By amount = By.xpath("//input[contains(@id, 'amount-inputEl')]");
		writeTextBoxByKeys(amount, unapliedamountText);
		...
		//PAYMENT TYPE
		By newPaymentLink = By.linkText("New");
		...
		String paymentInstrumentText = readFile.getCellValue(filepath, "Sheet6", 8, 1);
		By paymentInstrument = By.xpath("//input[contains(@id, 'PaymentMethod-inputEl')]");
		writeTextBoxByKeys(paymentInstrument, paymentInstrumentText);
		Thread.sleep(2000);
		
		String IBANText = readFile.getCellValue(filepath, "Sheet6", 9, 1);
		By IBAN = By.xpath("//input[contains(@id, 'IBAN-inputEl')]");
		writeTextBoxByKeys(IBAN, IBANText);
